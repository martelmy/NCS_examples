...
Sometimes pairing fails, and in those cases retrying will solve the issue most of the time. However, if you get BT Disconnected with reason 0x2a directly after BT Connected then you will not be able to connect and pair with your phone/tablet.
...
The current consumption when the device is not in lower power mode is around 50-60 mA. In low power mode the device should have periods of 100-200 ms where the current consumption is around 15 µA +- 3 µA. When pinging, you should get three spikes, one for each ping, of around 160 mA. If the device was in power saving mode before pings it should still be in power saving mode after.

<b>Strange spikes in Power Profiler</b>

Strange spikes in the Power Profiler of 100s of mA comes from the PPK2. If this happens once in a while it is fine, but if it starts happening often reboot the Power Profiler app (ctrl + R), restart sampling, and reboot the nRF7002 DK. 
...
A log from the device showing normal and expected operations can be found in [wifi_demo.log](wifi_demo.log). This log shows the device starting up, being provisioned to the network, power saving mode being enabled, and ping.
Resetting the device

<b>How to reprogram the device</b>

The precompiled firmware can be found in precompiled_hex. The merged_domains.hex file can be used to program both cores simultaneously using the nRF Connect Programmer app. The device can also be programmed using nrfjprog, programming each core separately.
To program with nrfjprog, open the precompiled_hex folder in a terminal and run the following:
```
nrfjprog -f NRF53 --coprocessor CP_NETWORK --program GENERATED_CP_NETWORK_merged_domains.hex --chiperase \
nrfjprog -f NRF53 --program GENERATED_CP_APPLICATION_merged_domains.hex --chiperase \
nrfjprog --pinreset
```

<b> Failed to allocate net_pkt </b>

The following errors can be ignored, explanation on what happens below:
```
...
```
This error comes from the wpa_supplicant. If `CONFIG_WNM` is defined and normal data traffic has not been sent recently, it will try to send a keep-alive to the access point. This is done by calling `l2_packet_send()` to send a packet with a length of 0. This causes issues with allocating net_pkt buffer due to `alloc_len` being equal to zero.
...
Sometimes programming and booting the device with PPK2 connected might fail. In those cases, disconnect the PPK2 before programming/booting the device, and reconnect it after the device har started up.
