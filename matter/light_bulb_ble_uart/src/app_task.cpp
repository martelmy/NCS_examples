...
void AppTask::ButtonEventHandler(uint32_t buttonState, uint32_t hasChanged)
{
	...
	if (BLE_NUS_BUTTON & hasChanged) {
		button_event.ButtonEvent.PinNo = BLE_NUS_BUTTON;
		button_event.ButtonEvent.Action =
			static_cast<uint8_t>((BLE_NUS_BUTTON_MASK & buttonState) ? AppEventType::ButtonPushed :
										    AppEventType::ButtonReleased);
		button_event.Handler = StartBLENUSHandler;
		PostEvent(button_event);
	}
}
...
{
	if (event.ButtonEvent.PinNo != BLE_NUS_BUTTON)
		return;
